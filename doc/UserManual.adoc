...
The options that can be set are:
    ...
    PRETTY_MULTILINE (TRUE or FALSE)
    PROVENANCE (TRUE or FALSE)
...
Sets the minimum subscript index to 1. The default is zero. The default can be restored with OPTION BASE 0.

OPTION HEADING TRUE

Displays the opening and closing lines that identify BASIC.

OPTION TIMING TRUE

After running a program, display time to execute (user time and system time).
...
OPTION PRETTY_MULTILINE TRUE

When listing programs, list each statement on a separate line.

OPTION PROVENANCE TRUE

When TRACE is TRUE, expand display of a variable to include the line in which it was assigned its value.
...
Option is either 'BASE', 'TRACE' or 'PROVENANCE'.
The options for HEADING and TIMING are not available as a run-time option.
...
Each will be executed as a normal statement.
