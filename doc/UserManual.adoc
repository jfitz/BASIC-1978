...
|--decimals digits |Sets the number of decimals printed. Default is 5, maximum is 15.
...
|--epsilon |Sets the minimum value for numeric variables. Values below epsilon are converted to zero. Default is 1E-7.
...
|ALLOW_ASCII |TRUE or FALSE |FALSE |Allow ASCII constants for control characters
|ALLOW_HASH_CONSTANT |TRUE or FALSE |FALSE |Allow hash constant values (#K)
|ALLOW_PI |TRUE or FALSE |FALSE |Allow PI constant
...
|BACKSLASH_SEPARATOR |TRUE or FALSE |TRUE |Backslash separates statements
|BANG_COMMENT |TRUE or FALSE |FALSE |Allow comments staring with exclamation point
...
|COLON_FILE |TRUE or FALSE |FALSE |Colon indicates a file handle (:2, :4, etc.)
|COLON_SEPARATOR| TRUE or FALSE |FALSE |Colon separates statements
|CRLF_ON_LINE_INPUT |TRUE or FALSE |FALSE |LINE INPUT reads CR/LF into variable
|DECIMALS |numeric 1 to 15 |5 |Sets the number of decimals printed
...
|EPSILON |numeric positive |1E-7 |Values below epsilon are converted to zero.
|FORNEXT_ONE_BEYOND |TRUE or FALSE |FALSE |FOR/NEXT ends with index value one higher than limit
...
|MIN_MAX_OP |TRUE or FALSE |FALSE |Allow MIN and MAX operators
...
This is unlike the MAT statements, which always begin with index 1.
...
This behavior is not changed with the OPTION BASE statement.
