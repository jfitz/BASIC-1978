...
|IF_FOR_SUB |TRUE or FALSE |TRUE |Allows a FOR statement to be a substatement of an IF statement
...
|INT_BITWISE |TRUE or FALSE |TRUE |Operators AND, OR, and NOT perform bitwise operations on integer values
...
|PRECISION |numeric 1 to 17, or "INFINITE" |9 |Sets the precision for numeric values
...
|LOG10(x) |Computes the base-10 log of x. A value of zero or a negative value will cause an error.
|LOG2(x) |Computes the base-2 log of x. A value of zero or a negative value will cause an error.
