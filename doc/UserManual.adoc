...
Expressions may be used to indicate the loop start, end, and step values.
...
If expressions are used, assignments in the loop to variables in those expressions will have no affect on the loop.
...
Examples:
	100 FOR I IN A
	100 FOR I IN A STEP 4
	100 FOR I STEP 2 IN A
	100 FOR I IN A STEP -1

The variable and array are names.
They cannot be expressions.
They cannot contain subscripts.

The control variable is assigned a value in each iteration of the loop.
Loops without STEP clauses iterate from the OPTION BASE to the upper bound of the array.
Loops with negative step clauses iterate from the upper bound down to the OPTION BASE.

During execution of a FOR/IN loop, the dimensions of the array variable are locked and cannot be chaged.
During the execution of a FOR/IN loop, the setting for OPTION BASE is locked and cannot be changed.
...
Sorts the array (internally) and returns the middle value.
If the array contains an even number of elements, it returns the lesser of the two center elements.
...
Extract substring of t from position p to the end of the string.
The first character is position 1.
If p is greater than the length of the string, the result is empty ("").
p must be positive.
