10 REM RAIL SIMULATION
11 OPTION REQUIRE_INITIALIZED TRUE
12 OPTION BASE 1
20 REM FUNCTIONS
21 REM FORMAT TIME INTO STRING
22 DEF FNH$(T) = RIGHT$("0"+STR$(INT(T/3600)),2)+":"+RIGHT$("0"+STR$(INT(MOD(T,3600)/60)),2)+":"+RIGHT$("0"+STR$(MOD(T,60)),2)
23 REM CALCULATE STOPPING TIME
24 DEF FNS(V,A) = 2.0*(ABS(V)/ABS(A))
25 REM CALCULATE STOPPING DISTANCE
26 DEF FND(V,A,T) = V*T + 0.5*(A)*T^2
90 REM NUMBER OF AND POSITIONS OF STATIONS (in meters)
91 DATA 2, 0, 8.0E3
95 REM NUMBER OF AND POSITIONS AND VALUES OF TRACK SPEED SIGNS
96 DATA 6, 0,5, 1.0E3,20, 1.5E3,40, 2.0E3,60, 2.5E3,30, 3.0E3,60
100 REM CONSTANTS
101 READ N0
110 DIM K(N0)
111 ARR READ K
120 REM CONSTANTS FOR TRAINSET
121 REM ACCELERATION (meters/second^2)
122 LET A0 = 1.0
123 REM MAX VELOCITY (meters/second)
124 LET V9 = 80
150 REM SPEED SIGNALS
151 READ P0
152 DIM P(P0,2)
153 MAT READ P
200 REM ONE TRIP
201 LET X = K(1)
202 LET V = 0
290 PRINT "T  ";"TIME ";"X  ";"V  ";"A  ";"T5 ";"X5 ";"K(2) ";"STATUS"
299 LET T = 17*60*60+2*60
1000 REM LOOP
1001 LET S$ = ""
1010 REM COMPUTE STOPPING TIME
1011 LET T5 = FNS(V, A0)
1020 REM COMPUTE STOPPING DISTANCE
1021 LET X5 = FND(V, A0, T5)
1200 REM IF CLOSE TO DESTINATION, SLOW DOWN
1201 IF X + X5 >= K(2) THEN 1204
1202 LET A = A0
1203 GOTO 1210
1204 LET A = -A0
1205 LET S$ = "APPROACHING STATION"
1210 REM SET MAX SPEED FROM SIGNAL
1211 FOR I=1 TO P0
1212 IF P(I,1) <= X THEN V8 = P(I,2)
1219 NEXT I
1290 REM SET MAX SPEED
1291 LET V5 = V9
1292 IF V5 > V8 THEN LET V5 = V8
1700 REM ADJUST SPEED
1711 IF A < 0 THEN 1750
1712 IF V > V5 THEN 1760
1720 REM CONTINUE OR INCREASE
1721 IF V = V5 THEN 1740
1730 REM INCREASE
1731 LET V = V + A
1739 GOTO 1900
1740 REM CONTINUE
1742 LET A = 0
1749 GOTO 1900
1750 REM BRAKE
1751 IF V <= 0 THEN 1900
1752 LET V = V + A
1753 LET S$ = S$ + " BRAKING"
1759 GOTO 1900
1760 REM BRAKE
1761 LET A = -A0
1762 LET V = V + A
1900 REM CHECK VELOCITY
1901 IF V >= 0 THEN 1960
1902 LET V = 0
1960 REM MOVE THE TRAIN
1961 LET X = X + V
1970 REM REPORT POSITION AND SPEED
1971 LET T$ = FNH$(T)
1972 PRINT T;T$;X;V;A;T5;X5;K(2);S$
1980 REM IF AT STOPPING POINT, STOP SIMULATION
1981 IF X >= K(2) THEN 9998
1990 REM STEP INTO FUTURE
1991 LET T = T + 1
1999 GOTO 1000
9998 STOP
9999 END
