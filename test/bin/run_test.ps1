...
If (Test-Path "$TESTROOT\$TESTGROUP\$TESTNAME\ref\analyze.txt") {
    ...
    Set-Location "$TESTBED\$TESTNAME"
    ruby basic.rb --analyze "$TESTNAME.bas" --print-width 0 >analyze.txt 2>analyze.err $GROUP_OPTIONS $TEST_OPTIONS $ANALYZE_OPTIONS
    Set-Location ..\..
...
If (Test-Path "$TESTROOT\$TESTGROUP\$TESTNAME\ref\stdout.txt") {
    ...
    If (Test-Path "$TESTBED\$TESTNAME\stdin.txt") {
    	Set-Location "$TESTBED\$TESTNAME"
	    Get-Content stdin.txt | ruby basic.rb --profile $OPTIONS --run "$TESTNAME.bas" --print-width 0 --echo-input >stdout.txt $GROUP_OPTIONS $TEST_OPTIONS $GROUP_RUN_OPTIONS $RUN_OPTIONS
	    Set-Location ..\..
    } else {
    	Set-Location "$TESTBED\$TESTNAME"
...
If (Test-Path "$TESTROOT\$TESTGROUP\$TESTNAME\ref\trace.txt") {
    ...
    If (Test-Path "$TESTBED\$TESTNAME\stdin.txt") {
    	Set-Location "$TESTBED\$TESTNAME"
	    Get-Content stdin.txt | ruby basic.rb $OPTIONS --run "$TESTNAME.bas" --print-width 0 --trace --echo-input >trace.txt $GROUP_OPTIONS $TEST_OPTIONS $GROUP_RUN_OPTIONS $RUN_OPTIONS
	    Set-Location ..\..
    } else {
    	Set-Location "$TESTBED\$TESTNAME"
