...
If (Test-Path "$TESTROOT\$TESTGROUP\$TESTNAME\ref\stdout.txt") {
    ...
    If (Test-Path "$TESTBED\$TESTNAME\stdin.txt") {
    	Set-Location "$TESTBED\$TESTNAME"
	    Get-Content stdin.txt | ruby basic.rb --profile $OPTIONS --run "$TESTNAME.bas" --print-width 0 >stdout.txt $GROUP_OPTIONS $TEST_OPTIONS $GROUP_RUN_OPTIONS $RUN_OPTIONS
	    Set-Location ..\..
    } else {
    	Set-Location "$TESTBED\$TESTNAME"
...
If (Test-Path "$TESTROOT\$TESTGROUP\$TESTNAME\ref\trace.txt") {
    ...
    If (Test-Path "$TESTBED\$TESTNAME\stdin.txt") {
    	Set-Location "$TESTBED\$TESTNAME"
	    Get-Content stdin.txt | ruby basic.rb $OPTIONS --run "$TESTNAME.bas" --print-width 0 --trace >trace.txt $GROUP_OPTIONS $TEST_OPTIONS $GROUP_RUN_OPTIONS $RUN_OPTIONS
	    Set-Location ..\..
    } else {
    	Set-Location "$TESTBED\$TESTNAME"
