...
class AbstractForModifier < AbstractModifier
  ...
  def initialize(tokens_lists, control_and_start_tokens, _step_tokens, _end_tokens, _until_tokens, _while_tokens)
    ...
    @variable = Variable.new(control_name, :scalar, [], [])
  ...
  def execute_pre_stmt(interpreter)
    ...
    unless @end.nil?
      ...
      fornext_control =
        ForToControl.new(@variable, from, step, to, @loopstart_line_stmt_mod)
    end
  ...
  def execute_post_stmt(interpreter)
    fornext_control = interpreter.retrieve_fornext(@variable)
...
class ForUntilModifier < AbstractForModifier
  ...
  def pre_pretty
    "FOR #{@variable} = #{@start} UNTIL #{@until}"
  end
end
...
class ForUntilStepModifier < AbstractForModifier
  ...
  def pre_pretty
    "FOR #{@variable} = #{@start} UNTIL #{@until} STEP #{@step}"
  end
end
...
class ForWhileModifier < AbstractForModifier
  ...
  def pre_pretty
    "FOR #{@variable} = #{@start} WHILE #{@while}"
  end
end
...
class ForWhileStepModifier < AbstractForModifier
  ...
  def pre_pretty
    "FOR #{@variable} = #{@start} WHILE #{@while} STEP #{@step}"
  end
end
